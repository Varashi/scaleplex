...
// dialectName maps a source backend to the matching dialect.backendName(), so
// "is this argv already native?" compares correctly. The NVIDIA source tag is
// "nvenc" (encoder-accurate) but the dialect is named "nvidia".
func (s sourceBackend) dialectName() string {
	switch s {
	case srcVAAPI:
		return "vaapi"
	case srcNVENC:
...
func reshapeForeignHWArgv(args []string, tm tonemapConfig) ([]string, []string) {
	...
	if src.dialectName() == activeDialect.backendName() {
		return args, nil // already native
	}
	d := activeDialect
