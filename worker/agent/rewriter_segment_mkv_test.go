...
func TestRewriter_PlexWindows_SegmentMkv_RewritesURL(t *testing.T) {
	...
	// Duration from -metadata regardless of is_live, and stock jellyfin
	// matroskaenc.c's IS_SEEKABLE = pb seekable && !is_live falls to the
	// else-branch at live=1 → 1 s / 32 KB cluster defaults = per-frame
	// clusters that Plex Windows needs.
	foundLive1 := false
	...
	if containsString(out.Changes, "hls:segment_format_options:live=1->live=0+per-frame-clusters") {
		t.Errorf("live=1 rewrite should be gone post-0094: %v", out.Changes)
	}
}
