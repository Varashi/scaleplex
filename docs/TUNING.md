...
| `SCALEPLEX_SUB_RENDER_HEIGHT` | `1080` | Caps the height at which the unified `inlineass` filter rasterises subtitles (libass for text, `replay_bitmap` for PGS/DVD bitmap). Renders inside the filter at that resolution then VPP-upscales onto the video — no separate pre-render, no FIFO, no `overlay_vaapi`. libass / bitmap render cost scales ~linearly with pixel area, so at 4K the `1080` default cuts in-filter CPU ~4.25× for marginally softer glyph edges. Outputs ≤ the cap are unaffected (native). Tiers: `720` (~9× cheaper, visibly soft), `1080` (~4.25×, clean), `1440` (~2.25×, indistinguishable from native). Set `0` to opt out (native full-res render). | Raise to `1440`/`0` if 4K burned-in subs look too soft on your displays; lower to `720` only under heavy CPU pressure. Passed as `inlineass=…:render_height=N` by the rewriter. |
