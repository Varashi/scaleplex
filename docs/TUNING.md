...
| `SCALEPLEX_TONEMAP_ALGO` | `hable` | Tone-mapping algorithm for sessions where Plex's argv carries no tonemap filter of its own (HW-decode passthrough / SW shapes — the rewriter injects the stage). One of `hable`, `mobius`, `reinhard`, `bt2390`, `linear`, `gamma`, `clip`. When Plex *does* send a `tonemap_opencl` chain, its algorithm is honored and this knob is ignored. No effect when `SCALEPLEX_TONEMAP=vaapi`. | Pick the curve you prefer for HDR10→SDR. `hable` (filmic) is the common choice; `bt2390` matches the old fixed-curve look. |
