...
| `SCALEPLEX_TONEMAP` | `opencl` | HDR→SDR tone-mapping backend. `opencl` runs `tonemap_opencl` (algorithm-selectable, honors Plex's `TranscoderTonemapAlgorithm`) — costs ~15% throughput vs the fixed curve, still ~10× realtime at 4K HDR→1080p on an Arc A310. `vaapi` uses iHD's fixed BT.2390 EETF `tonemap_vaapi` instead. | Set to `vaapi` to fall back to the fixed curve (no rebuild needed) if an OpenCL ICD is missing on a worker or an algorithm renders poorly. |
| `SCALEPLEX_TONEMAP_ALGO` | `hable` | Fallback tone-mapping algorithm — one of `hable`, `mobius`, `reinhard`, `bt2390`, `linear`, `gamma`, `clip`. Algorithm precedence: a `tonemap_opencl` chain in Plex's argv → Plex's `TranscoderToneMappingAgorithm` pref (the shim reads it from `Preferences.xml`) → **this knob** → `hable`. So in practice this only applies when the shim couldn't read the Plex pref. No effect when `SCALEPLEX_TONEMAP=vaapi`. | Rarely needs tuning — Plex's own pref normally wins. Set it as a cluster-wide default if the shim can't reach `Preferences.xml`. |

scaleplex also **honors Plex's `Enable HDR tone mapping` toggle**: the shim surfaces `TranscoderToneMapping` from `Preferences.xml`, and when it is off the rewriter skips the implicit tonemap injection entirely (matching Plex's own behavior — washed/dim output, by the user's choice). No env var — it follows the Plex server setting.
...
| `SCALEPLEX_PLEX_PREFS` | (auto-discovered) | Set on the **PMS pod** (the shim's environment) to override the path to Plex's `Preferences.xml`. The shim otherwise looks under `$PLEX_MEDIA_SERVER_APPLICATION_SUPPORT_DIR/Plex Media Server/` then `/config/Library/Application Support/Plex Media Server/`. Only needed for a non-standard Plex data dir. |
