...
| 0124 | `libavfilter/vf_tonemap_cuda.c` | — | scaleplex (Plex argv capture 2026-05-27) | Reorders `tonemap_cuda`'s AVOption array so `format` (output pixel format, string) sits at positional 2, matching Plex's fork. Without this, PMS argv `tonemap_cuda=hable:nv12` (HW-decode-passthrough HDR-tonemap, emitted verbatim) hits jellyfin's positional-2 = `tonemap_mode` (enum: max/rgb/lum/itp/auto) and fails with `Undefined constant or missing '(' in 'nv12'`. Named-arg form `tonemap_cuda=tonemap=hable:format=nv12` works on both forks and is what the worker rewriter emits on reshape paths (scaleplex PR #61 `14c7f45`); positional shape is required for HW-decode-passthrough where the rewriter doesn't touch `-filter_complex`. All other AVOptions remain accessible by name (lookup is position-agnostic); `tonemap_mode` is never set explicitly in our 209-entry corpus so the demotion is a safe trade. |
| 0125 | `libavcodec/{nvenc.h, nvenc_h264.c, nvenc_hevc.c}` | — | scaleplex (T2 NVIDIA replay 2026-05-28) | NVENC counterpart to 0118 (VAAPI). When the worker swaps `libx264`/`libx265` to `h264_nvenc`/`hevc_nvenc` on force-HW / bare-HW-upgrade sessions it leaves PMS's `-preset NAME` + `-x264opts`/`-x265-params` untouched, but NVENC's `preset` is an INT enum so libx264 names fail option parsing (`Undefined constant or missing '(' in 'veryfast'`); blocked 28/70 cases. Unlike 0118 (string preset mapped at init), NVENC can't reach init with a string, so the missing libx264 names are registered as `AV_OPT_TYPE_CONST` in the existing `.unit = "preset"` namespace, each resolving to a p1..p7 enum value that the native `nvenc_map_preset()` then routes to a preset GUID + pass count (no init code): ultrafast/superfast/veryfast→p1, faster→p2, slower→p6, veryslow/placebo→p7. fast/medium/slow are NOT added — already native nvenc names mapped to p1/p4/p7. Adds the constants to both per-codec tables (no shared macro) plus `x264opts`/`x265-params` string sinks (accepted and ignored) backed by two new `char*` fields on `NvencContext`. |
